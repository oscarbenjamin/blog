...
the structure of a computer algebra system (CAS) like SymPy and describe
SymPy's current problems. Then subsequent posts will focus in more detail on
particular components and the work that has been done and what should be done
in the future.

I am writing this with the intention that it should be accessible to someone
who is not a SymPy developer although to some extent other SymPy developers are
the intended audience for many of the points that I will make. Many of the
things that I will describe here are not well understood even by many SymPy
developers though and a major goal of this series of posts is to help to try to
change that.
...
contributors and users. Some like Sage are directed more at pure
...
this is not a huge problem even if it does cause some confusion for beginners.
...
Traditionally these other situations are handled by a CAS and so in the context
of scientific computing the usual distinction is between working "numerically"
with floating point numbers or otherwise using a CAS to work "symbolically" or
"analytically". There is a lot more to a CAS than this but in simple terms this
is the role that SymPy plays within the Python scientific computing ecosystem.
...
SymPy those are all built on top of these three components.

A robust numerical evaluation subsystem
---------------------------------------

Above I use the word "numerical" to mean calculations that are done with
machine precision floating point numbers. This is the usual meaning of the word
but this use of the word is tricky because in the context of a CAS we can also
do numerical calculations with arbitrary precision approximate or exact numbers
and those are also "numerical" calculations. I will define several levels of
numerical calculation ranging from machine precision floating point to exact
and symbolic calculations. As we go down this list the calculations become
...
- Arbitrary precision floating point: This is a kind of floating point number
  that can have as many digits of precision as we want. These calculations are
  much slower than machine precision floating point calculations but by
  increasing the precision we can make the calculations as accurate as we want.
  The ``mpmath`` library that is used by SymPy is an example of this.
- "Robust" floating point calculations: These are calculations that are done
  ...
  be correct to within a specified tolerance. SymPy's ``evalf`` is an example
  of *heuristic* robust numerics and the ``arb`` library that I will discuss
  later is an example of a library that does *formal* robust numerics.
...
automatically realise from the above list that ``mpmath`` and ``evalf`` (which
uses ``mpath``) are at different levels in this scheme. As an example to
demonstrate the difference we will compute :math:`\sin(1)^2 + \cos(1)^2 - 1`
using SymPy's ``evalf``, mpmath and also SymEngine which is a C++ library that
recreates the "core" of SymPy in C++:
...
a fundamental limitation of arbitrary precision numerics as compared to robust
numerics: we can use as many digits as we like and the result will likely be
accurate but without very careful analysis we generally do not know *anything*
about how accurate the result is. It is always possible that had we used more
digits we would have found that the result was not zero after all but mpmath
does not indicate any level of uncertainty about the result.
...
will still only use arbitrary *fixed* precision and not *robust* numerics.
...
SymEngine's ``evalf``. However these levels are not interchangeable and SymPy's
other two core systems (the expression system and the computational algebra
subsystem) absolutely *do* depend on SymPy's ``evalf`` giving *robust* numeric
evaluation and not just arbitrary precision numeric evaluation: many things
would break if SymPy's ``evalf`` was changed to behave like SymEngine's
``evalf``.
...
expression system or on the algorithms that operate on these expressions. In
...
entirely impossible to answer this question: unbounded compution can occur
...
any actual computation (many SymPy contributors *do* think if it as free but
...
and all of their subclasses builds automatic evaluation into the core data
structures of the symbolic subsystem. This is a fundamental design flaw that
causes all kinds of problems for speed, behaviour and extensibility. Using
SymEngine (in its current form) can make this subsystem faster but it would
then make all of the *other* problems unsolvable.
...
code and contributions would not be wasted but would have to be adapted over
time to a new framework.
...
SymPy's ability to do anything useful. In a CAS that was aimed more at pure
mathematics this subsystem would be the most prominent feature but in SymPy it
is mostly hidden away and not well understood. Few SymPy users or developers
know how to use the lower levels of this system and few are aware of the
features that it provides or why you might want to use them instead of the
...
can represent more complex expressions. I have described this subsystem in
...
``plotting``, and ``stats``. The "polynomials" module is the largest module in
SymPy making up over 10% of the all of the code and is used by *everything*
else but is sorely undervalued by the SymPy community.
...
before my own involvement in SymPy it looks like it was well maintained by
...
the future. It also looks though as if those people at some point just moved on
leaving the code in the middle of a redesign that was never finished which is
very confusing for anyone who comes to that part of the codebase now.
...
in a lower level language like C. It is clear from the code that this was the
plan at some point but that has not yet happened. Happily we do not need to
rewrite everything in C ourselves because there is now already a C library that
does exactly what we need: Flint. I have been working on making it possible for
SymPy to leverage Flint which is a library for doing fast computations with
polynomials and matrices in the way that SymPy's computational algebra
subsystem does.
...
- The computational algebra subsystem should be made faster by leveraging Flint
  and should be used more in place of the symbolic subsystem.
