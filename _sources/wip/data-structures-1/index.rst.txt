...
requires ``f`` to be an expression but if ``f(x)`` is an expression then ``f``
is a head. The fact that heads are classes and expressions are instances means
that a head cannot be an expression. Instead we have awkward workarounds using
things like ``Subs``:
...
*subclass* of ``Basic`` rather than an *instance* of ``Basic``. To make ``f``
seem like an expression (even though it is not) ``Function`` has to be a
*metaclass* but that leads to other problems. There is a long-standing SymPy
issue about fixing it so that functions are actually expressions in their own
right [sympy_issue_functions_objects]_.
...
There are many more problems with the ``Basic`` design but I will just list a
few more in the remaining paragraphs of this section. Firstly the ``Basic``
design is a constant source of bugs. The design works reasonably well if
contributors stick to its rules and do not try to mess with it in subclasses.
The "rules" are not really documented anywhere though and actually many
experienced contributors do not even know what they are. I do not think I have
seen any new contributor understand how to implement a subclass without getting
significant things wrong and without trying to mess with some of the methods
that should not be messed with. The problem is that classes are code and you
can do anything you want in that code. I am not sure that anyone has a good
enough understanding of all of the methods of ``Basic`` to actually implement
them correctly for a new subclass. Many of the methods need to be implemented
quite carefully and many are not implemented correctly already (the codebase is
full of examples of what *not* to do!).

Attaching all of the code to the classes means that the combined code for an
important operation like "differentiation" is distributed all over the codebase
in different classes and modules and many different methods. In some cases
these problems have been realised such as the printing code where instead all
of the printing code was removed from all of the classes and centralised in one
place. The problem then is that the printing code itself is not easily
extensible to add new types of expression which means that now to add a new
expression you need to subclass ``Basic`` to add the new expression and then
subclass the printers to add the handlers to print your new type of expression.
...
of any of those classes from within SymPy's Python codebase or to do all of
...
can switch to different data structures for different operations. Again that
means *not* using classes for expression heads.
...
.. [no_proper_way_to_recurse_evalf] https://github.com/sympy/sympy/issues/20479
